...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.vgg > 0:
            self.vgg_loss = networks_ver4.PerceptualLoss(opt)
            if self.opt.IN_vgg:
                self.vgg_patch_loss = networks_ver4.PerceptualLoss(opt)
                self.vgg_patch_loss.cuda()
            self.vgg_loss.cuda()
            self.vgg = networks_ver4.load_vgg16("./model", self.gpu_ids)
            self.vgg.eval()
            for param in self.vgg.parameters():
                param.requires_grad = False
        elif opt.fcn > 0:
            self.fcn_loss = networks_ver4.SemanticLoss(opt)
            self.fcn_loss.cuda()
            self.fcn = networks_ver4.load_fcn("./model")
            self.fcn.eval()
            for param in self.fcn.parameters():
                param.requires_grad = False
        # load/define networks_ver4
        # The naming conversion is different from those used in the paper
        # Code (paper): G_A (G), G_B (F), D_A (D_Y), D_B (D_X)

        skip = True if opt.skip > 0 else False
        self.netG_A = networks_ver4.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids, skip=skip, opt=opt)
        # self.netG_B = networks_ver4.define_G(opt.output_nc, opt.input_nc,
        #                                 opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids, skip=False, opt=opt)

        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD_A = networks_ver4.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, self.gpu_ids, False)
            if self.opt.patchD:
                self.netD_P = networks_ver4.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
        ...
        if self.isTrain:
            ...
            if opt.use_wgan:
                self.criterionGAN = networks_ver4.DiscLossWGANGP()
            else:
                self.criterionGAN = networks_ver4.GANLoss(use_lsgan=not opt.no_lsgan, tensor=self.Tensor)
            if opt.use_mse:
        ...
        print('---------- networks_ver4 initialized -------------')
        networks_ver4.print_network(self.netG_A)
        # networks_ver4.print_network(self.netG_B)
        if self.isTrain:
            networks_ver4.print_network(self.netD_A)
            if self.opt.patchD:
                networks_ver4.print_network(self.netD_P)
            # networks_ver4.print_network(self.netD_B)
        if opt.isTrain:
            self.netG_A.train()
            # self.netG_B.train()
        else:
