...
class Self_Attn(nn.Module):
    ...
    def __init__(self,in_dim,activation,mod):
        ...
        self.query_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//mod , kernel_size= 1)
        self.key_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//mod , kernel_size= 1)
...
class Attn_conv(nn.Module):
    def __init__(self):
        ...
        self.attn = Self_Attn(3, activation = nn.ReLU())

    def forward(self, input):
        input_ds = self.pool(input)
        attn0, _ = self.attn(input_ds)

        attn1_1 = self.bn1_1(self.ReLU(self.conv1_1(attn0)))
        # attn1_2 = self.bn1_2(self.ReLU(self.conv1_2(attn1_1)))
        attn1 = self.pool(attn1_2)
