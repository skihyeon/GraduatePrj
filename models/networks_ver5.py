...
import numpy as np
from .Attention_ver5 import Self_Attn
# from Attention import Attn_conv
###############################################################################
# Functions
###############################################################################
...
class Unet_resize_conv(nn.Module):
	def __init__(self, opt, skip):
		...
		self.attn0 = Self_Attn(3, activation=nn.ReLU(), mod=3)
		self.attn1 = Self_Attn(32, activation=nn.ReLU(), mod=8)
		self.attn2 = Self_Attn(64, activation=nn.ReLU(), mod=8)
		self.attn3 = Self_Attn(128, activation=nn.ReLU(), mod=8)
		self.attn4 = Self_Attn(256, activation=nn.ReLU(), mod=8)
		self.attn5 = Self_Attn(512, activation=nn.ReLU(), mod=8)
