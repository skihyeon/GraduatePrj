...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        #Self Attn block
        self.attn = Self_Attn(512, activation  = nn.ReLU())
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            ...
            # ver1.
            # x_attn, _ = self.attn(conv5)
            # attn = F.upsample(x_attn, scale_factor=2, mode='bilinear')
            # up6 = torch.cat([self.deconv5(attn), conv4], 1)
            ...
            attn = F.upsample(x_attn, scale_factor=2, mode='bilinear')
            cl = torch.cat([attn, conv5],1)
            attn_cat = self.deconv_attn(cl)
            up6 = torch.cat([self.deconv5(attn_cat), conv4], 1)
            #

            # # ver3.
            # x_attn, _ = self.attn(conv5)
            # cl = torch.cat([x_attn, conv5], 1)
            # up_attn = F.upsample(cl, scale_factor=2, mode='bilinear')
            # attn = self.deconv_attn(up_attn)
            # up6 = torch.cat([self.deconv5(attn), conv4], 1)
            # # 
